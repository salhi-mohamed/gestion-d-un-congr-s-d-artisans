    ...
    do {
        ...
        switch (toupper(reponse)) {
            case 'N':
                ...
                break;
            case 'P':
                cout << "Saisir le nouveau pr�nom : ";
                cin >> prenom;
                break;
            case 'T':
                cout << "Saisir le nouveau num�ro de t�l�phone : ";
                cin >> tel;
                break;
            case 'E':
...
void personne::supprimer_langue() {
    ...
    // Demander � l'utilisateur le num�ro de la langue � supprimer
    int choix;
    cout << "Entrez le num�ro de la langue � supprimer : ";
    cin >> choix;

    // V�rifier si le num�ro est valide
    if (choix < 1 || choix > langues.size()) {
        cout << "Num�ro invalide. Op�ration annul�e." << endl;
        return;
    }

    // Supprimer la langue s�lectionn�e
    langues.erase(langues.begin() + choix - 1);

    // Mettre � jour nblangues
    nblangues = langues.size();

    cout << "Langue supprim�e avec succ�s." << endl;
}
